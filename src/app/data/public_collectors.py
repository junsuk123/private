...
class RssNewsCollector:
    ...
    def collect_with_articles(
        ...
        for index, item in enumerate(root.findall(".//item")):
            ...
            body = description
            article_record = None
            if fetch_articles and _within_limit(index, article_limit) and link.startswith(("http://", "https://")):
                try:
        ...
        return RssCollectionResult(events=tuple(events), raw_records=tuple(raw_records))
...
class YahooChartMarketDataCollector:
    ...
    def collect_latest(
        ...
        payload = self.client.get_json(url, {"range": "1mo", "interval": "1d", "includePrePost": "true"})
...
class AlphaVantageDailyMarketDataCollector:
    ...
    def collect_latest(
        ...
        if "Error Message" in payload:
            raise DataCollectionError(str(payload["Error Message"]))
        if "Information" in payload and "Time Series (Daily)" not in payload:
            raise DataCollectionError(str(payload["Information"]))

        series = payload.get("Time Series (Daily)", {})
        if not isinstance(series, dict) or not series:
            raise DataCollectionError(f"no alpha vantage daily data for {symbol}")
        ...
        if not closes:
            raise DataCollectionError(f"missing alpha vantage close prices for {symbol}")
        returns = [
            (closes[index] - closes[index - 1]) / closes[index - 1]
            for index in range(1, len(closes))
            if closes[index - 1]
        ...
        source = source_now(
            "alpha_vantage_daily",
            f"{url}?function=TIME_SERIES_DAILY&symbol={symbol}",
            f"alpha-vantage-daily:{_stable_id(symbol)}",
        )
        return MarketSnapshot(
            ticker=ticker,
            market=market,
...
def _within_limit(index: int, limit: int | None) -> bool:
    return limit is None or index < max(0, limit)
