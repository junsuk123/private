...
class KeywordEventClassifier:
    ...
    def classify(self, title: str, body: str, ticker: str | None = None) -> EventClassifierResult:
        ...
        labels = [
            ...
            if _matches_label(label, text, keywords)
        ]
        if not labels:
            labels = ["unknown"]
...
def _matches_label(label: str, text: str, keywords: tuple[str, ...]) -> bool:
    if label != "supply_contract":
        return any(item in text for item in keywords)
    # "contract" alone is common in regulatory and legal prose. Require both
    # a supply/order concept and an agreement/award concept for this label.
    supply_terms = (
        "supply",
        "supplier",
        "procurement",
    ...
    agreement_terms = ("contract", "agreement", "deal", "award", "order")
    return any(term in text for term in supply_terms) and any(
        term in text for term in agreement_terms
    )
