...
# 캘린더
# --------------------------------------------------------------------------- #
class _ExternalExchangeCalendars:
    """Small, tri-state adapter around ``exchange_calendars``.

    Calendars are resolved eagerly.  This avoids a race where concurrent first
    requests could observe a half-initialized provider, and lets construction
    fail cleanly back to the bundled snapshot.
    """

    _NAMES = {MarketGroup.KR: "XKRX", MarketGroup.US: "XNYS"}

    def __init__(self) -> None:
        import exchange_calendars

        self._calendars = {
            group: exchange_calendars.get_calendar(name)
            for group, name in self._NAMES.items()
        }

    def covers(self, group: MarketGroup, day: date) -> bool:
        calendar = self._calendars[group]
    ...
    def is_holiday(self, group: MarketGroup, day: date) -> bool | None:
        if not self.covers(group, day):
            return None
        return not bool(self._calendars[group].is_session(day.isoformat()))
...
class CalendarSnapshot:
    ...
    external: _ExternalExchangeCalendars | None = None
...
def _t(text: str) -> time:
    # NXT starts its main session at 09:00:30. Rounding to a minute incorrectly
    # permits a route during the KRX-only opening interval.
    value = time.fromisoformat(str(text))
    if value.tzinfo is not None:
        raise ValueError("session wall-clock times must not contain a UTC offset")
    return value
...
_DEFAULT_WINDOWS: dict[SessionId, SessionWindow] = {
    ...
    SessionId.NXT_REGULAR: SessionWindow("Asia/Seoul", _t("09:00:30"), _t("15:20")),
    # 15:30-15:40 accepts auction orders but has no continuous execution.
    # Auction-entry trading is not implemented; continuous routes begin at 15:40.
    SessionId.NXT_POST: SessionWindow("Asia/Seoul", _t("15:40"), _t("20:00")),
...
def _calendar_from_mapping(raw: object) -> CalendarSnapshot:
    ...
    if bool(data.get("prefer_external_provider", True)) and _external_calendar_provider_available():
        try:
            external = _ExternalExchangeCalendars()
        except Exception:  # noqa: BLE001 - a broken provider must fall back atomically.
            external = None
        else:
...
class MarketSessionService:
    ...
    def calendar_state(self, group: MarketGroup, now_utc: datetime | None = None) -> tuple[str, ...]:
        ...
        external_covers = bool(
            self.calendar.external is not None
            and self.calendar.external.covers(group, local_day)
        )
        if not self.calendar.covers(local_day) and not external_covers:
            reasons.append(ReasonCode.SESSION_CALENDAR_STALE.value)
        if not self.calendar.is_complete(group) and not external_covers:
    ...
    def _build_capability(
        ...
        start_local = local.replace(
            ...
            second=window.start.second, microsecond=0
        )
